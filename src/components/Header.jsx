  ...
  const UserMenu = () => (
    <div className="relative" ref={userMenuRef}>
      ...
      >
        ...
        <ChevronIcon isOpen={userMenuOpen} />
      </button>
      <AnimatePresence>
        {userMenuOpen && (
          <motion.div
            ...
            className="absolute right-0 top-full mt-1 bg-gray-100 dark:bg-slate-800 rounded-lg shadow-xl border border-gray-200 dark:border-white/10 py-1 min-w-[200px]"
          >
            ...
            <div className="border-t border-gray-200 dark:border-white/10 mt-1 pt-1">
              <button
                ...
                className="w-full px-4 py-2 text-gray-600 dark:text-gray-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-all flex items-center gap-2 text-sm"
              >
                <span>🚪</span> Logout
              </button>
            </div>
            ...
            <div className="px-4 py-3 border-t border-gray-200 dark:border-white/10 bg-white/95 dark:bg-slate-900/95">
                      ...
                      >
                        {item.children.map((child) => (
                          <Link
                            ...
                            onClick={() => { setIsMenuOpen(false); setMobileOpenDropdown(null); }}
                          >
                            <span>{child.icon}</span>
                            <span>{child.label}</span>
                          </Link>
              ...
              <div className="border-t border-gray-200 dark:border-white/10 mt-2 pt-2">
                <button onClick={toggleTheme}
                  className="flex items-center gap-2 px-3 py-2 rounded-lg text-gray-600 dark:text-gray-300 text-sm hover:bg-gray-100 dark:hover:bg-white/5 w-full">
                  <ThemeIcon /> <span className="capitalize">{theme}</span>
                </button>
              </div>
              <UserMenuMobile />
            </div>
